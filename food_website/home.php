...
// Get the parameter from the URL (if needed)
...
// Now you can use $_SESSION['contact_number'] wherever needed
?>
    <center>
        <pre><h3>Welcome to Xpress Hotel</h3></pre>
    </center>

    <div class="d-flex align-items-start justify-content-center text-right mt-3 pt-">
...
//         let menuName = card.querySelector('.card-title b').innerText || card.querySelector('.card-title').innerText;
...
// </script>
            ...
            <!-- <script>
              // Get the menu details when the ADD button is clicked
                    ...
                    let amoun  function addToCart(button) {
                    t = quantity * parseFloat(rate);
                    let menuTypeId = card.querySelector('img').src.includes('veg_icon.png') ? 1 : 2;
        ...
        `;
            ...
            <script>
                let currentMenuId = null; // To track the MenuID of the current item added to the cart
...
function addToCart(button) {
    ...
    // Insert the record into the database using submit_order.php
    fetch('submit_order.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
function decreaseQuantity(button) {
    let input = button.nextElementSibling;
    if (parseInt(input.value) > 1) { // Prevent quantity from going below 1
        input.value = parseInt(input.value) - 1;
        updateOrder(button);
    }
}
